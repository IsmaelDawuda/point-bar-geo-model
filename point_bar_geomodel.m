function [corner_point_grid,well_locs_locations,lac_x,lac_y,ihs_hor,ihs_vert] = point_bar_geomodel(filename,loc_wells,nx,ny,nz,PBH,DTOP,property_SGSIM)
...
conv_factor =  .3048;    %to conversion factor from ft to m; this will be converted back to ft in the final stage;
...
channel_x      =  conv_factor .*channel_x; 
channel_y      =  conv_factor .*channel_y; 
ch_points      =  [channel_x;channel_y]; 

loc_wells(1,:) =  conv_factor.*loc_wells(1,:);
loc_wells(2,:) =  conv_factor.*loc_wells(2,:);
PBH            =  conv_factor*PBH;
...
% ------------FIND SLOPES BTN EACH MEANDER AND THE FOCUS, CALC THE CORDINATES AT THE TOE OF THE CHANNELS-------
 ...
 lam     =   (lambda/10.9);
 W       =   nthroot(lam,1.01);
 Wo      =   W/1.5;                 %(Leopold and Wolman, 1960), this is d unit is in meters
 d       =   Wo;
...
lac_x = finalx(:,:,end);   lac_x = reshape(lac_x,ny+1,nx+1);
...
my_coordinates     =   [xcmg ycmg  (DTOP +flipud(zcmg))];  

%  ---- BACK TRANSFORM COORDINATES
my_coordinates = my_coordinates'; 

back_trans_pts  = zeros(3,numel(my_coordinates(1,:)));
...
save('corner_point_grid.txt','corner_point_grid','-ascii');
...
well_locs_locations = (1/conv_factor)*back_trans_well_locs;  % well locations converted back to feets
save('well_locs_locations.txt','back_trans_well_locs','-ascii');

% --- DISPLAYING POINT BAR WITH ITS POROSITY DISTRIBUTION---

property = property_SGSIM; 

nr    =   1;           %number of realisations
n     =   nx*ny*nz;     % number of grid blocks in one realisation
r_n   =   1;            % realisation umber 
NN    =   n*nr;                                                             % total number of grid blocks in all realisations                    
property_reshaped    =    reshape(property,n,nr);

property_center      =    property_reshaped(1:end,r_n);  % porosity from SGSIM was modeled on a block-centred grid

property_center      =    reshape(property_center, nx,ny,nz);


property_corner = zeros(nx+1,ny+1,nz+1);  % corner point format;  
for i = 1:points_along_curve-1
    for j = 1:N_P -1
        for k =1:points_alongsigmoid-1
            
        property_corner(i,j,k) =  property_center(i,j,k);
...
property_corner(end, 1:end,1:end) = property_corner(end-1, 1:end,1:end);

property_corner(1:end, end,1:end) = property_corner(1:end, end-1,1:end);

property_corner(1:end, 1:end,end) = property_corner(1:end, 1:end,end-1);



    
phi_res = reshape(property_corner, points_along_curve*N_P,points_alongsigmoid);
...
aa = 1:numel(filename(:,1));aa=aa'; b = num2str(aa); c = cellstr(b);
 dx = 6; dy =6; % displacement so the text does not overlay the data points, it can be adjusted depending on the data given
%text(channel_x+dx4, channel_y + dy4, c);
text(filename(:,1)+dx, filename(:,2)+dx, c);
...
a = 1:numel(channel_x);a=a'; b = num2str(a); c = cellstr(b);
 dx = 6; dy =6; % displacement so the text does not overlay the data points, it can be adjusted depending on the data given
%text(channel_x+dx4, channel_y + dy4, c);
text(channel_x+dx, channel_y+dx, c);
...
% ---POROSITY DISTRIBUTION WITHIN THE 3D POINT BAR  
figure(5);
surf(Resx,Resy,Resz,phi_res);
%EdgeColor = 'none';
colormap(jet); 
colorbar
shading flat
...
title 'Point Bar Porosity Distribution'
...
% phi_center = reshape(property, nx*ny,nz);
% 
% Resx_center    =  reshape(xcoord_center_rect , nx*ny,nz);
% Resy_center    =  reshape(ycoord_center_rect , nx*ny,nz);
% Resz_center    =  reshape(zcoord_center_rect , nx*ny,nz);
%  
% surf(Resx_center,Resy_center,Resz_center,phi_center);
% %EdgeColor = 'none';
% colormap(jet); 
% colorbar
% shading flat
...
mypor      =   property_corner(mysection,1:end,1:end);
Rx         =   reshape(myfx,N_P,points_alongsigmoid);
Ry         =   reshape(myfy, N_P,points_alongsigmoid);
Rz         =   reshape(myfz, N_P,points_alongsigmoid);
mypor_res  =   reshape(mypor,N_P,points_alongsigmoid);



%s = surf(myRy,myRz,ensm_res);
surf(Ry,Rz,mypor_res);
shading flat
colormap(jet)
c = colorbar;
% %c.Label.String = 'Permeability^{2}(md^{2})';
