...
to  model a gridded point bar geometry, and assigns its petrophysical property distributions obtained from Geostatistical simulation.

The coordinates of wells should be in feets. 



 The "geogrid.m" function models the point bar geometry and generates a grid for the point bar; to activate this function, run the "Demo_grid_only.m" file  

 The "point_bar_geomodel.m"  function performs all the functions of the 'geogrid.m' function and assigns properties to the point bar grid; to activate this function, run the "Demo.m" file
...
property_SGSIM :        %the property data which has been modeled using GEOSTATISTICS (SGSIM and ELLIPSIM ). This is only required when running the "Demo.m" file

Once the above input parameters are specified, the code can be run
...
the final curvilinear grid and equivalent rectilinear grid for the point bar geometry will be saved  as a text file in an ascii format as follows:
 a) 'curv_corner_grid.txt'  : curvinear grid in a corner point grid format
 b) 'curv_center_grid.txt'  : curvinear grid in a block-centered grid format 
 c) 'rect_corner_grid.txt'  : rectilinear grid in a corner point grid format
 d) 'rect_center_grid.txt'  : rectilinear grid in a block-centered grid format 
...
4) Property distribution for the Point Bar in 3D (If "Demo.m" file is run)
